...
function revenuefeesinsert($name, $roll_no, $amount, $class, $date, $address, $status, $guardian_name, $added_by, $payment_type, $payment_id, $paidBy){

    $sql = "INSERT INTO `revenue_studentfees` ( `name`, `roll_no`, `amount`, `class`,`date`, `address`, `status`, `guardian_name`,  `added_by`, `payment_type`, `payment_id`, `paid_by`) VALUES ('$name', '$roll_no', '$amount', '$class', '$date', '$address', '$status', '$guardian_name', '$added_by', '$payment_type','$payment_id', '$paidBy')";
...
function revenueothersinsert($source, $amount, $date, $status, $added_by, $payment_type, $payment_id, $vendor_name, $vendor_id, $paidBy){

    $sql = "INSERT INTO `revenue_others` ( `source`, `amount`, `date`, `status`, `added_by`, `payment_type`, `payment_id`, `vendor_name`, `vendor_id`,  `paid_by`) VALUES ('$source', '$amount', '$date', '$status', '$added_by', '$payment_type', '$payment_id', '$vendor_name', '$vendor_id', '$paidBy')";
...
function revenueStudentdisplay(){
    ...
    $sql = "SELECT * FROM `revenue_studentfees` order by id desc";
...
function studentfeesDay(){
    ...
    $sql="SELECT DAYNAME(amount) AS 'amount', SUM(amount) AS 'Total' FROM `revenue_studentfees` where day(DATE)=(SELECT day(CURDATE())) and month(DATE)=(SELECT MONTH(CURDATE())) AND year(DATE)=(SELECT YEAR(CURDATE())) ORDER BY DAY(amount);";
...
function studentfeesMonth(){
    ...
    $sql="SELECT MONTHNAME(amount) AS 'amount', SUM(amount) AS 'Total' FROM `revenue_studentfees` where month(DATE)=(SELECT MONTH(CURDATE())) AND year(DATE)=(SELECT YEAR(CURDATE())) ORDER BY MONTH(amount);";
...
function studentfeesYear(){
    ...
    $sql="SELECT YEAR(amount) AS 'amount', SUM(amount) AS 'Total' FROM `revenue_studentfees` where year(DATE)=(SELECT YEAR(CURDATE())) ORDER BY YEAR(amount);";
...
function studentfeesTotal(){
    ...
    $sql="SELECT  amount AS 'amount', SUM(amount) AS 'Total' FROM `revenue_studentfees`;";
...
function revenuechartday($expenses_chart){
    ...
    $sql = "SELECT * FROM `revenue_studentfees` where day(DATE)=(SELECT day(CURDATE())) and month(DATE)=(SELECT MONTH(CURDATE())) AND year(DATE)=(SELECT YEAR(CURDATE())) ORDER BY DAY(amount)";
...
function studentreportmonth($expenses_chart){
    ...
    $sql = "SELECT * FROM `revenue_studentfees` where month(DATE)=(SELECT MONTH(CURDATE())) AND year(DATE)=(SELECT YEAR(CURDATE())) ORDER BY MONTH(amount)";
...
function studentreportyear($expenses_chart){
    ...
    $sql = "SELECT * FROM `revenue_studentfees` where year(DATE)=(SELECT YEAR(CURDATE())) ORDER BY YEAR(amount);";
...
function updatestudentrevenue($Id){
    ...
    $sql = "SELECT * FROM revenue_studentfees WHERE `revenue_studentfees`.`id` = '$Id'";
...
function studentrevenueE($name, $roll_no, $address, $class, $amount, $id, $status, $guardian_name, $payment_type, $date, $payment_id, $paid_by){

    $sqledit = "UPDATE  `revenue_studentfees` 
                SET `name` = '$name',
                `roll_no` = '$roll_no',
                `address`= '$address',
                `class`= '$class',
...
function studentfeescancel($id){
    ...
    $sql2 = "UPDATE  `revenue_studentfees` SET `status` = 'inactive' WHERE `revenue_studentfees`.`id` = '{$id}'";
...
function studentfeesactive($id){
    ...
    $sql2 = "UPDATE  `revenue_studentfees` SET `status` = 'active' WHERE `revenue_studentfees`.`id` = '{$id}'";
...
//   inshat dada start w
   ...
   // departnemt action update start w
    ...
    function editOtherRevenue($source, $amount, $status, $id, $paid, $others_paid, $payment_type, $payment_id){

        $sqledit = "UPDATE  `revenue_others` SET `source` = '$source', `amount`= '$amount', `status` = '$status', `id` = '$id', `paid` = '$paid', `others_paid` = '$others_paid', `payment_type` = '$payment_type', `payment_id` = '$payment_id'  WHERE `revenue_others`.`id` = '$id'";
        $result1 = $this->conn->query($sqledit);
        return $result1;
...
function studentsearchdata($searchstudents, $searchstudents1){
    ...
    $sqldal = "SELECT * FROM `revenue_studentfees` WHERE concat(`date`) between '$searchstudents' to '$searchstudents1'";
...
function studentDetails($searchstudents1, $searchstudents){
    ...
    $sql = "SELECT * FROM `revenue_studentfees` WHERE concat(`date`)  between '$searchstudents1' and '$searchstudents'";
...
$sql="SELECT MONTHNAME(amount) AS 'amount', SUM(amount) AS 'Total' FROM `revenue_studentfees` WHERE concat(`date`)  between '$searchstudents1' and '$searchstudents'";
...
$sql=" SELECT MONTHNAME(date) as chatname, SUM(amount) as amount FROM `revenue_studentfees` where year(DATE)=(SELECT YEAR(CURDATE())) GROUP BY chatname";
...
//         $sql = "INSERT INTO `revenue_studentfees` ( `name`, `roll_no`, `amount`, `class`,`date`, `address`, `status`, `added_by`, `payment_type`, `payment_id`, `paid`, `others_paid`) VALUES ('$name', '$roll_no', '$amount', '$class', '$date', '$address', '$status', '$added_by', '$payment_type','$payment_id', '$paid','$others_paid')";
...
//         $sqledit = "UPDATE  `revenue_studentfees` SET `name` = '{$name}', `roll_no` = '{$roll_no}', `address`= '{$address}', `class`= '{$class}', `amount`= '{$amount}', `id`= '{$id}', `status` = '{$status}'  WHERE `revenue_studentfees`.`id` = '{$id}'";
...
//         $sql2 = "UPDATE  `revenue_studentfees` SET `status` = '0' WHERE `revenue_studentfees`.`id` = '{$id}'";
