...
class StudentDetails extends DatabaseConnection{
  ...
  // INDEX display start w
    ...
    function showStudentmark($subject_id, $student_id, $session){

        $sql = "SELECT marks FROM `subject_marks` WHERE `subject_id` = '$subject_id' and `student_id` = '$student_id' and `session` = '$session' ";
    $studentTypeQuery = $this->conn->query($sql);
    $rows = $studentTypeQuery->num_rows;
    if ($rows == 0) {
    return 0;
...
function Studentmarktotal($student_id, $session){

    $sql = "SELECT  SUM(marks) AS 'Total' FROM `subject_marks` WHERE `student_id` = '$student_id' and `session` = '$session' ";
$studentTypeQuery = $this->conn->query($sql);
$rows = $studentTypeQuery->num_rows;
if ($rows == 0) {
return 0;
...
return $data;
    ...
    function numberOfClass($showstu){

        $sql = "SELECT COUNT(class_name) as classnumber FROM `class_subject` WHERE `class_name` = '$showstu'";
