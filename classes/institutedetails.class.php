...
// inshat data end
    ...
    function classInsert( $classname, $description){

        $sql = "INSERT INTO `class` (`classname`, `description`) VALUES ( '$classname', '$description')";
    ...
    function classupdate($classname, $description, $id){

        $sqledit = "UPDATE  `class` SET `classname` = '$classname', `description`= '$description' WHERE `class`.`id` = '$id'";
    ...
    function marksTotal($marksdata, $session, $student_id, $class_id, $total, $marks){

         

        $sql = "SELECT * FROM `total_marks` WHERE `total_marks`.`student_id` = '$student_id' and `total_marks`.`class` = '$class_id' and `total_marks`.`session` = '$session'";
         ...
         if ($rows == 0) {
             ...
             $sql = "INSERT INTO `total_marks` (`marks`, `session`, `student_id`, `class`) VALUES ( '$marks', '$session', '$student_id', '$class_id')";
             ...
             else
             {
                $data = $marksdata - $marks;
                 ...
                 $sqledit = "UPDATE  `total_marks` SET `marks` = '$data' WHERE  `total_marks`.`student_id` = '$student_id' and `total_marks`.`class` = '$class_id' and `total_marks`.`session` = '$session'";
